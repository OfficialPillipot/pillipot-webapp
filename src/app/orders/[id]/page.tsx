...
export default function OrderDetailsPage() {
    ...
    return (
      <div className="flex flex-col min-h-screen bg-pp-surface">
        ...
        <div className="flex-1 pp-container py-20 text-center">
          <LuCircleX className="w-20 h-20 text-red-100 mx-auto mb-6" />
  ...
  const getStatusInfo = (status: string) => {
    switch (status.toLowerCase()) {
      case "delivered": return { label: "Delivered", icon: <LuCircleCheck className="w-6 h-6 text-pp-success" />, color: "text-pp-success" };
      ...
      <main className="flex-1 pp-container py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* Status Card */}
            <div className="bg-white rounded-3xl p-4 sm:p-8 border border-gray-100 shadow-sm relative overflow-hidden">
               <div className="absolute top-0 left-0 w-2 h-full bg-pp-primary" />
               <div className="flex items-center justify-between gap-4 mb-6 sm:mb-8">
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="p-2 sm:p-3 bg-pp-surface rounded-2xl">
                      {statusInfo.icon}
                    </div>
                    <div>
               ...
               {/* Status Progress */}
               <div className="relative pt-4 pb-10 px-2 sm:px-4">
                  {/* Background track */}
                  <div className="absolute top-8 left-[calc(4%+16px)] right-[calc(4%+16px)] h-1 bg-gray-100" />
                  {/* Progress fill */}
                  <div 
                    className="absolute top-8 left-[calc(4%+16px)] h-1 bg-pp-primary transition-all duration-1000" 
            ...
            {order.trackingId && (
              <div className="flex flex-col gap-3 rounded-2xl border border-pp-cyan/20 bg-pp-surface-alt p-5 sm:flex-row sm:items-center sm:gap-4">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-pp-primary/10">
                  <LuTruck className="h-5 w-5 text-pp-primary" />
                </div>
                ...
                <button
                  ...
                  className="flex items-center gap-2 self-start rounded-xl border border-pp-cyan/20 bg-white px-4 py-2 text-xs font-black uppercase tracking-wider text-pp-primary transition-all hover:bg-pp-primary hover:text-white sm:self-center"
                >
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-4 sm:px-8 py-4 sm:py-6 border-b border-gray-50">
                <h2 className="text-lg sm:text-xl font-black text-gray-900">Order Items</h2>
              </div>
                ...
                {order.items.map((item: any, idx: number) => (
                  <div key={idx} className="p-4 sm:p-8 flex gap-4 sm:gap-6 items-start flex-wrap sm:flex-nowrap">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {order.status === "delivered" && (
                        <button 
                          ...
                          className="mt-3 flex items-center gap-2 text-pp-primary text-xs font-black uppercase tracking-widest hover:text-pp-primary-dark transition-colors"
                        >
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-lg sm:text-xl font-black text-pp-primary">{formatPrice(item.sellingAmount + (item.deliveryFee || 0))}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-8">
            {/* Delivery Info */}
            <div className="bg-pp-dark text-white rounded-3xl p-5 sm:p-8 shadow-xl">
              <div className="flex items-center gap-3 mb-5 sm:mb-6">
                <LuMapPin className="w-5 h-5 text-pp-accent-warm" />
                <h2 className="text-base sm:text-lg font-black uppercase tracking-tighter">Shipping Address</h2>
              </div>
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-start gap-3 sm:gap-4">
                  <LuUser className="w-4 h-4 text-gray-500 mt-1 shrink-0" />
                  <p className="font-bold text-gray-200 text-sm sm:text-base">{order.customerName}</p>
                </div>
                <div className="flex items-start gap-3 sm:gap-4">
                  <LuMapPin className="w-4 h-4 text-gray-500 mt-1 shrink-0" />
                  <div className="text-gray-300 text-xs sm:text-sm font-medium leading-relaxed">
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-8 border border-gray-100 shadow-sm">
              <h2 className="text-lg sm:text-xl font-black text-gray-900 mb-5 sm:mb-6">Order Summary</h2>
              <div className="space-y-3 sm:space-y-4 text-sm font-bold mb-5 sm:mb-6">
                ...
                <div className="flex justify-between text-base sm:text-lg font-black text-pp-primary">
                  <span>Total Amount</span>
                  <span>{formatPrice(order.grandTotal)}</span>
                </div>
              </div>
              <div className="p-3 sm:p-4 bg-gray-50 rounded-2xl flex items-center gap-3">
                <LuCreditCard className="w-5 h-5 text-gray-400 shrink-0" />
                <div>
